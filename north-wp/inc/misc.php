...
function thb_quick_search() {
	if (ot_get_option('header_search', 'on') === 'on') {
		?>
		<a href="#searchpopup" rel="inline" data-class="quick-search" id="quick_search"><?php esc_html_e('Search', 'north'); ?></a>
...
function thb_newsletter() {
	if(!is_admin() && ($popup = ot_get_option('newsletter') == 'on')) {
		$cookie = isset($_COOKIE['newsletter_popup']) ? wp_unslash($_COOKIE['newsletter_popup']) : false;
		if (!$cookie) {
		...
		<aside id="newsletter-popup" rel="inline-auto" class="mfp-hide theme-popup newsletter-popup" data-class="newsletter-popup">
			...
			<div class="newsletter-content">				
				<?php if ($newsletter_content) { echo do_shortcode($newsletter_content); } ?>
				<?php if ($newsletter_form === 'theme-form') { ?>
				<form id="newsletter-form" class="newsletter-form">   
					<input type="text" name="email" id="widget_subscribe" class="full" placeholder="<?php esc_attr_e("Your e-mail address",'north'); ?>">
					<input type="submit" name="submit" class="btn" value="<?php esc_attr_e("Subscribe Now",'north'); ?>" />
					<div class="result"></div>
				</form>
				<?php } else { echo do_shortcode($newsletter_form_shortcode); } ?>
			</div>
		</aside>
		<?php
		}
...
function thb_get_cookie_length() {
...
function thb_newsletter_cookie() {
	$popup_interval = ot_get_option('newsletter-interval', 1);
	$time = thb_get_cookie_length();
	
	if ($time) {
		if (isset($_COOKIE['newsletter_popup'])) {
			if ($_COOKIE['newsletter_popup'] !== $popup_interval) {
	...
	<aside class="share-article">
		...
		<div class="icons">
			<div class="inner">
				...
				<a href="whatsapp://send" data-text="<?php echo htmlspecialchars(urlencode(html_entity_decode($title, ENT_COMPAT, 'UTF-8')), ENT_COMPAT, 'UTF-8') . ''; ?>" class="whatsapp social" data-href="<?php echo esc_url( $permalink ); ?>" data-action="share/whatsapp/share"><i class="fa fa-whatsapp"></i></a>
				<?php } ?>
			</div>
		</div>
	</aside>
