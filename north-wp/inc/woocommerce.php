...
function thb_quick_cart() {
	...
	?>
		<a id="quick_cart" data-target="open-cart" href="<?php echo esc_url(wc_get_cart_url()); ?>" title="<?php esc_attr_e('Cart','north'); ?>">
			<?php esc_html_e('Cart','north'); ?> <span class="float_count"><?php echo esc_html(WC()->cart->get_cart_contents_count()); ?></span>
		</a>
	<?php
	}
}
...
function thb_wishlist_button() {
    ...
    $html .= '><a href="' . esc_url( add_query_arg( 'add_to_wishlist', $product->get_id() ) ) . '" data-product-id="' . esc_attr($product->get_id()) . '" data-product-type="' . esc_attr($product_type) . '" class="' . esc_attr($classes) . '"><span class="text">'.esc_html__( "Add To Wishlist", 'north' ).'</span>'.thb_load_template_part('assets/img/svg/wishlist.svg').'</a>';
    $html .= '</div>';
		
		$html .= '<div class="yith-wcwl-wishlistaddedbrowse hide" style="display:none;"><a href="' . esc_url(esc_url($url)) . '"><span class="text">'.esc_html__( "Added to Wishlist", 'north' ).'</span>'.thb_load_template_part('assets/img/svg/wishlist.svg').'</a></div>';
		
		$html .= '<div class="yith-wcwl-wishlistexistsbrowse ' . ( $exists ? 'show' : 'hide' ) . '" style="display:' . ( $exists ? 'block' : 'none' ) . '"><a href="' . esc_url($url) . '"><span class="text">'.esc_html__( "View Wishlist", 'north' ).'</span>'.thb_load_template_part('assets/img/svg/wishlist.svg').'</a></div>';
...
/* Sharing */
add_action('woocommerce_single_product_summary', 'thb_share', 39);
